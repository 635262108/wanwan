...
$(function(){
	...
	//获取content对象里面，数据的数量
	var number_of_items = $('.indContainerContent').children(".indContainer").size();

	//计算页面显示的总页数
	var pageCount = Math.ceil(number_of_items/show_per_page);

	//隐藏该对象下面的所有子元素
	$('.indContainerContent').children(".indContainer").css('display', 'none');

	//显示第n（show_per_page）元素
	$('.indContainerContent').children(".indContainer").slice(0, show_per_page).css('display', 'flex');
	...
	$("#pageGro li").live("click",function(){
			...
			$('.indContainerContent').children(".indContainer").css('display', 'none').slice(start_from, end_on).css('display', 'flex');
		if(pageCount > 5){
			//显示页面
